...
?>
        <tr class="uni-wishlist-table-row<?php if ( $i == 1 ) { echo ' row-odd'; } else { echo ' row-even'; } ?>">
            ...
            <td>
                <?php
                if ( empty($aBridallistItemData['variations'][$iProductVariationId]) ) {
                if ( $oProduct->is_in_stock() ) :

$sNewVariationProductAddToCartUrl = add_query_arg( array('bridallist_id' => $iUserId), $oProduct->add_to_cart_url() );
echo apply_filters( 'woocommerce_loop_add_to_cart_link',
	sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="button %s product_type_%s">%s</a>',
		esc_url( $sNewVariationProductAddToCartUrl ),
...
$oProduct );
            ...
            <td>
                ...
                if ( empty($aBridallistItemData['variations'][$iProductVariationId]) ) {
                    ...
                    echo '<a href="#" class="uni-wishlist-table-remove-link" data-pid="'.$iProductId.'" data-vid="'.$iProductVariationId.'" data-action="uni_bridallist_delete_from_list">'.$sRemoveItemTitle.'</a>';
                }
                ?>
            </td>
        </tr>